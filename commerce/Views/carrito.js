...
function obtenerDirecciones() {
    $.ajax({
        ...
        success: function(response) {
            ...
            var direccionesHtml = '<option class="text-dark rounded" style="border:none;" value="">Selecciona una dirección</option>';
            
            for (var i = 0; i < direcciones.length; i++) {
                var direccionCompleta = direcciones[i].direccion + ', ' + direcciones[i].localidad + ', ' + direcciones[i].provincia
                direccionesHtml += '<option class="text-dark rounded" style="border:none;"  value="' + direcciones[i].localidad + ', '+ direcciones[i].provincia + '">' + direccionCompleta + '</option>';
            }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var i = 0; i < cartItems.length; i++) {
                ...
                cartItemsHtml += '<tr class="text-dark">';
                ...
                cartItemsHtml += '<td class="text-dark"><input type="number" class="cantidadInput text-dark rounded" style="padding: 0px 10px; border:none; background: rgba(80,80,80,0.1);" value="' + cartItems[i].cantidad + '" min="1" max="' + cartItems[i].stock + '" data-id="' + cartItems[i].id + '"></td>';
                cartItemsHtml += '<td class="text-dark">' + itemSubtotal + '</td>';
                if (cartItems[i].cantidad > cartItems[i].stock) {
                    cartItemsHtml += '<td class="text-dark"><i class="fas fa-exclamation-triangle" title="La cantidad en el carrito es mayor que el stock disponible"></i></td>';
                } else {
                    cartItemsHtml += '<td class="text-dark"></td>';
                }
                cartItemsHtml += '<td class="text-dark"><button class="btn btn-danger delete-button" data-id="' + cartItems[i].id + '"><i class="fas fa-trash-alt"></i></button></td>';
                cartItemsHtml += '</tr>';
            }
